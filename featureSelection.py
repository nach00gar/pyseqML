...
def featureSelectMIFS(df, target, n_selected=None):
    feat_selector = mifs.MutualInformationFeatureSelector(method = "JMI", n_jobs=-1, n_features=results.shape[1])
    feat_selector.fit(df, target)
    feature_importances = feat_selector.mi_
    df_feature_importances = pd.DataFrame({'Importance': feature_importances}, index=results.columns[feat_selector.ranking_])
    sorted_features = df_feature_importances.sort_values(by='Importance', ascending=False)
    return sorted_features
...
def featureSelect(df, target, n_selected, method):
    if method == 'mRMR':
        ...
        return featureSelectMIFS(df, target, n_selected)
    else:
